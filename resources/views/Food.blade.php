...
@section('main')
    <div class="page-wrapper">
        <div class="page-content">
            <a href="{{ route('add.food') }}"><button class="btn btn-primary">Add Food</button></a>
            <div class="card">
                <div class="card-body">
                                        ...
                                        <tbody>
                                            @if ($foods)
                                                ...
                                                <tr role="row" class="even">
                                                    <td class="sorting_1">{{ $food->name }}</td>
                                                    <td>{{ $food->category->name }}</td>
                                                    <td>{{ $food->desc }}</td>
                                                    <td>{{ $food->image }}</td>
                                                    <td><a href="{{ route('edit.food', $food->id) }}"><button
                                                                class="btn btn-primary">Edit</button></a></td>
                                                    <td><button class="btn btn-primary"
                                                            onclick="Delete({{ $food->id }})">Delete</button></td>
                                                </tr>
                                            @endforeach
                                            @endif
...
@section('customJs')
    <script>
        function Delete(id) {
            $.ajax({
                url: '{{ route("delete.food",":id") }}'.replace(':id',id),
